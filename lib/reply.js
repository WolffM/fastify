...
function setupResponseListeners (reply) {
  ...
  const onResFinished = err => {
    ...
    // Clear socket._meta references to prevent memory leak on keep-alive connections
    const socket = reply.request.raw.socket
    if (socket && socket._meta) {
      socket._meta.context = null
      socket._meta.request = null
      socket._meta.reply = null
    }
